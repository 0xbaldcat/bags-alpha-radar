...
export async function getTokenSnapshotFromDb(mint: string): Promise<TokenSnapshot | null> {
  ...
  const token: BagsToken = {
    ...
    marketCapUsd: Number(row.token.marketCapUsd ?? 0),
    liquidityUsd: Number(row.token.liquidityUsd ?? 0),
    lifetimeFeesSol: Number(row.token.lifetimeFeesSol ?? 0),
    lifetimeFeesIndexed: Boolean(row.token.lifetimeFeesUpdatedAt),
